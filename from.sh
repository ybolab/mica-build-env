#!/usr/bin/env bash
# Turn images.env keys into the --build-arg lines that carry a
# `FROM` into a Dockerfile, and refuse everything that must not reach one.
#
#   bash from.sh MICA_BASE_IMAGE=IMAGE_DEBIAN_TRIXIE
#       -> --build-arg MICA_BASE_IMAGE=debian:trixie-slim@sha256:d7e12182...
#   bash from.sh --ref IMAGE_DEBIAN_TRIXIE
#       -> debian:trixie-slim@sha256:d7e12182...
#   bash from.sh --check
...
[ -e "${IMAGES_ENV}" ] || {
    echo "error: ${IMAGES_ENV} does not exist; from.sh reads the pins next to itself" >&2
    exit 1
}
...
check_image_key() {
    ...
    case "${v}" in
    *PENDING*)
        echo "error: ${k}=${v} in images.env is still PENDING, so there is no recorded base image to build on. Run \`bash build.sh\`: it resolves the tag, prints the digest and fails, which is how a digest gets recorded here" >&2
        return 1
        ;;
    esac
    if [ "${v#*@}" = "${v}" ]; then
        echo "error: ${k}=${v} in images.env names a TAG and not a digest. A tag is repointed by upstream whenever it rebuilds; pin it as name:tag@sha256:<64 hex> (see the HOW TO BUMP A DIGEST note in that file)" >&2
        return 1
    fi
    if ! [[ "${v}" =~ ^[a-z0-9][a-z0-9._/-]*:[A-Za-z0-9._-]+@sha256:[0-9a-f]{64}$ ]]; then
        echo "error: ${k}=${v} in images.env is not a well-formed digest pin. Expected name:tag@sha256: followed by exactly 64 lowercase hex digits" >&2
        return 1
    fi
    return 0
}
...
check_local_key() {
    ...
    localhost/*) ;;
    *)
        echo "error: ${k}=${v} in images.env does not start with localhost/, but its LOCAL_ prefix says it is an image this repository builds. One of the two is wrong" >&2
        return 1
        ;;
    esac
...
resolve_key() {
    ...
    val="${!key-}"
    if [ -z "${val}" ]; then
        echo "error: images.env defines no ${key}. Every base image in this tree is a key in that file; if this is a new one, add it there rather than writing it into a FROM or a docker run" >&2
        return 1
    fi
    case "${key}" in
    ...
    LOCAL_*)
        ...
        ;;
    *)
        echo "error: ${key} is neither an IMAGE_ nor a LOCAL_ key, so from.sh cannot say what would make it valid. A base image is either an upstream reference pinned by digest (IMAGE_) or one this repository builds (LOCAL_)" >&2
        return 1
        ;;
    esac
    printf '%s\n' "${val}"
...
if [ "${1-}" = "--check" ]; then
    ...
    [ "${seen}" -gt 0 ] || {
        echo "error: images.env defines no IMAGE_ key at all, so this check passed by having nothing to check" >&2
        exit 1
    }
    exit "${bad}"
fi
...
if [ "${1-}" = "--ref" ]; then
    shift
    [ "$#" -eq 1 ] || {
        echo "error: --ref takes exactly one images.env key. It prints one reference on stdout; a call with none would print an empty string that a caller would substitute into a docker command line as no image at all, and a call with several would have to be read back by position" >&2
        exit 1
    }
    resolve_key "$1"
    exit 0
